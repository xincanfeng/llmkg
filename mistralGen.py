...
local_model_path = "hfmodels/Mistral-7B-Instruct-v0.2"
...
class LocalLLM:
    def __init__(self):
        if quantization_config:
            ...
            self.model = AutoModelForCausalLM.from_pretrained(local_model_path, token=hf_token, quantization_config=quantization_config, device_map="auto", torch_dtype=torch.float16, attn_implementation="flash_attention_2")
        else:
            ...
            self.model = AutoModelForCausalLM.from_pretrained(local_model_path, token=hf_token, device_map="auto", torch_dtype=torch.float16, attn_implementation="flash_attention_2")
    ...
    def eval_accuracy(self, not_matched_triples: List[Tuple[str, str, str]], chunks: List[str], enable=['none']) -> Tuple[List[Tuple[str, str, str]], List[Tuple[str, str, str]]]:
        ...
        for gen_triple in not_matched_triples:
            ...
            messages = [
                {"role": "user", "content": f"You are an expert. Please judge if the following knowledge graph triple is correct or not based on the provided document segment.{add}"},
            ...
            model_inputs = self.tokenizer.apply_chat_template(messages, max_length=4096, return_tensors="pt", padding=True).to(device)
            generated_ids = self.model.generate(model_inputs, max_new_tokens=30, use_cache=True, do_sample=True)
            raw_answer = self.tokenizer.batch_decode(generated_ids)[0]
    ...
    def eval_confliction(self, similar_gt_triples_dict, chunks: List[str], enable=['none']) -> Tuple[dict, dict]:
        ...
        for gt_triple, gen_triples in similar_gt_triples_dict.items():
            ...
            model_inputs = self.tokenizer.apply_chat_template(messages, max_length=4096, return_tensors="pt", padding=True).to(device)
            generated_ids = self.model.generate(model_inputs, max_new_tokens=500, use_cache=True, do_sample=True)
            raw_conflict_output = self.tokenizer.batch_decode(generated_ids)[0]
            raw_conflict_output = self.remove_prompt(raw_conflict_output)
            # print(f"raw_conflict_output: {raw_conflict_output}")
    ...
    def eval_exact_equiv(self, equivalent_gt_triples_dict) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
        ...
        for gen_triple, equiv_gt_triples in equivalent_gt_triples_dict.items():
            ...
            for equiv_gt_triple in equiv_gt_triples:
                ...
                model_inputs = self.tokenizer.apply_chat_template(messages, max_length=400, return_tensors="pt", padding=True).to(device)
                generated_ids = self.model.generate(model_inputs, max_new_tokens=30, use_cache=True, do_sample=True)
                raw_answer = self.tokenizer.batch_decode(generated_ids)[0]
                raw_answer = self.remove_prompt(raw_answer)
    ...
    def eval_equivalence(self, similar_gt_triples_dict_w_promptlink) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
        ...
        for gen_triple, similar_gt_triples in similar_gt_triples_dict_w_promptlink.items():
            ...
            for similar_gt_triple in similar_gt_triples:
                messages = [
                    {"role": "user", "content": "You are an expert. Please judge if the following two triples are equivalent. Please answer by providing 'TRUE' or 'FALSE' value."},
                ...
                model_inputs = self.tokenizer.apply_chat_template(messages, max_length=300, return_tensors="pt", padding=True).to(device)
                generated_ids = self.model.generate(model_inputs, max_new_tokens=30, use_cache=True, do_sample=True)
                raw_answer = self.tokenizer.batch_decode(generated_ids)[0]
                raw_answer = self.remove_prompt(raw_answer)
                # print(f"raw_answer: {raw_answer}")
    ...
    def get_similar_entities_dict_w_promptlink(self, keywords_expander: str, k_similar_entities_dict_w_sapbert: Dict[str, List[str]], dataset: str) -> List[str]:
        ...
        for gen_entity, similar_gt_entities in k_similar_entities_dict_w_sapbert.items():
            ...
            for similar_gt_entity in similar_gt_entities:
                messages = [
                    {"role": "user", "content": f"You are an expert. Please judge if the following query entity and the candidate entity are similar and ensure that your decision adheres to these criteria:\n{filter_entity_criteria}. Please answer by providing 'TRUE' or 'FALSE' value."},
    ...
    def eval_similar_w_promptlink(self, keywords_expander, similar_gt_triples_dict, dataset: str) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
        ...
        for gen_triple, similar_gt_triples in similar_gt_triples_dict.items():
            ...
            for similar_gt_triple in similar_gt_triples:
                messages = [
                    {"role": "user", "content": f"You are an expert. Please judge if the following two triples are similar and ensure that your decision adheres to these criteria:\n{filter_entity_criteria}. Please answer by providing 'TRUE' or 'FALSE' value."},
                    {"role": "assistant", "content": "Okay, which two triples do you want me to compare?"},
    ...
    def iterative_denoising_and_keyword_extraction(self, keywords: List[str], chunks: List[str], iteration_times: int = 3) -> Tuple[List[str], List[List[str]]]:
        ...
        for iteration in range(1, iteration_times + 1):
            ...
            for chunk in tqdm(current_chunks, desc=f"Keywords Extraction Iter {iteration}"):
                messages = [
                    {"role": "user", "content": f"You are a scientific researcher. Please transform the text into a list of important keywords.\nText:\n{chunk}."},
                    {"role": "assistant", "content": "To better assist you, could you specify your criteria for extracting keywords?"},
                    {"role": "user", "content": f"Ensure the keyword extraction adheres to these criteria: {keyword_extraction}."},
                ]
                input_ids = self.tokenizer.apply_chat_template(messages, max_length=4096, return_tensors="pt", padding=True).to(device)             # Encoding the input messages
                generated_ids = self.model.generate(input_ids, max_new_tokens=1024, use_cache=True, do_sample=True)          # Generating the output
                out = self.tokenizer.batch_decode(generated_ids)[0]
                response_text = self.remove_prompt(out)
                extracted_keywords = parse_keywords_from_llm(response_text)
            ...
            for chunk, keywords in tqdm(zip(current_chunks, current_keywords), desc=f"Denoising Iter {iteration}", total=len(current_chunks)):
                denoised_chunks = []
                messages = [
                    {"role": "user", "content": f"You are a scientific researcher. Please denoise the raw text with the given keywords.\nText:\n{chunk}\nKeywords:\n{keywords}."},
                    {"role": "assistant", "content": "To better assist you, could you specify your criteria for denoising text?"},
                    {"role": "user", "content": f"Ensure the text denoising adheres to these criteria: {chunks_denoising}."},
                ]
                input_ids = self.tokenizer.apply_chat_template(messages, max_length=4096, return_tensors="pt", padding=True).to(device)             # Encoding the input messages
                generated_ids = self.model.generate(input_ids, max_new_tokens=1024, use_cache=True, do_sample=True)          # Generating the output
                out = self.tokenizer.batch_decode(generated_ids)[0]
                response_text = self.remove_prompt(out)
                denoised_chunks.append(response_text)
    ...
    def generate_kg(self, keywords_expander: str, dataset: str, keywords: List[str], chunks: List[str], enable=['none']) -> str:
        """
        ...
        messages = [
            {"role": "user", "content": f"You are a scientific researcher. Please generate a knowledge graph starting from the provided keywords and document segments. Keywords: {keywords}. Document segments: {chunks}."},
        ...
        generated_ids = self.model.generate(input_ids, max_new_tokens=2048, use_cache=True, do_sample=True)          # Generating the output
        # print(generated_ids)
