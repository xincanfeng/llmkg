...
def ask_gemini(request_text, max_retries=50):
    raw_answer = ''
    retries = 0
    ...
    while retries < max_retries:
        try:
            ...
            if response and response.text:
                ...
                break  # If valid response is obtained, exit the retry loop
            else:
                logger.warning(f"Invalid or incomplete response. Retrying... ({retries + 1}/{max_retries})")
        except Exception as e:
            logger.error(f"Error occurred when processing: {e}. Retrying... ({retries + 1}/{max_retries})")
        retries += 1
    return raw_answer
...
def eval_accuracy(not_matched_triples: List[Tuple[str, str, str]], chunks: List[str], enable=['none']) -> Tuple[List[Tuple[str, str, str]], List[Tuple[str, str, str]]]:
    ...
    for gen_triple in not_matched_triples:
        ...
        request_text = f"You are an expert at evaluating the accuracy of scientific knowledge graph based on the provided documents segment. Based on the document segments provided, please determine the correctness of the knowledge graph triple.{add} The knowledge graph triple:\n{gen_triple}\nThe document segment:\n{chunks}\nRespond with only one word: 'TRUE' or 'FALSE' and do not provide any explanation or reasoning."
        raw_answer = ask_gemini(request_text)
...
def eval_confliction(similar_gt_triples_dict, chunks: List[str], enable=['none']) -> Tuple[dict, dict]:
    ...
    for gt_triple, gen_triples in similar_gt_triples_dict.items():
        ...
        chunks_str = "\n".join(chunks)
        request_text = conflict_resolution_prompt.format(triples_str=triples_str, chunks_str=chunks_str)
        raw_conflict_output = ask_gemini(request_text)
    ...
    for gen_triple, equiv_gt_triples in equivalent_gt_triples_dict.items():
        ...
        for equiv_gt_triple in equiv_gt_triples:
            request_text = (
                ...
                "Please judge if the following two triples are exactly equivalent. Triple 1: {gen_triple}, Triple 2: {equiv_gt_triple}. Please answer by providing 'TRUE' or 'FALSE' value.")
            raw_answer = ask_gemini(request_text)
...
def eval_equivalence(similar_gt_triples_dict_w_promptlink) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
    ...
    for gen_triple, similar_gt_triples in similar_gt_triples_dict_w_promptlink.items():
        ...
        for similar_gt_triple in similar_gt_triples:
            request_text = "You are an expert at evaluating scientific knowledge graphs equivalence who always responds by providing 'TRUE' or 'FALSE' value. Please judge if the following two triples are equivalent. Triple 1: {gen_triple}, Triple 2: {similar_gt_triple}."
            raw_answer = ask_gemini(request_text)
...
def get_similar_entities_dict_w_promptlink(keywords_expander: str, k_similar_entities_dict_w_sapbert: Dict[str, List[str]], dataset: str) -> List[str]:
    ...
    for gen_entity, similar_gt_entities in k_similar_entities_dict_w_sapbert.items():
        ...
        for similar_gt_entity in similar_gt_entities:
            request_text = f"You are an expert at evaluating entity similarity in scientific knowledge graphs based on these criteria:\n{filter_entity_criteria}. You always respond by providing 'TRUE' or 'FALSE' value. Please judge if the following two entities are similar. The query entity: {gen_entity}, the candidate entity: {similar_gt_entity}."
            raw_answer = ask_gemini(request_text)     
...
def eval_similar_w_promptlink(keywords_expander, similar_gt_triples_dict, dataset: str) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
    ...
    for gen_triple, similar_gt_triples in similar_gt_triples_dict.items():
        ...
        for similar_gt_triple in similar_gt_triples:
            request_text = f"You are an expert at evaluating triples similarity in scientific knowledge graphs based on these criteria:\n{filter_entity_criteria}. You always respond by providing 'TRUE' or 'FALSE' value. Please judge if the following two triples are similar. The query triple: {gen_triple}, the candidate triple: {similar_gt_triple}"
            raw_answer = ask_gemini(request_text)          
...
def iterative_denoising_and_keyword_extraction(keywords: List[str], chunks: List[str], iteration_times: int = 3) -> Tuple[List[str], List[List[str]]]:
    ...
    for iteration in range(1, iteration_times + 1):
        ...
        for chunk in tqdm(current_chunks, desc=f"Keywords Extraction Iter {iteration}"):
            request_text = f"You are a scientific knowledge graph constructor who is trained to transform the text into a list of important keywords based on these criteria:\n{keyword_extraction}\n\nPlease transform the text into a list of important keywords.\nText:\n{chunk}."
            out = ask_gemini(request_text) 
        ...
        for chunk, keywords in tqdm(zip(current_chunks, current_keywords), desc=f"Denoising Iter {iteration}", total=len(current_chunks)):
            denoised_chunks = []
            request_text = f"You are a scientific knowledge graph constructor who is trained to denoise the raw text with the given keywords based on these criteria:\n{chunks_denoising}\n\nPlease denoise the raw text with the given keywords.\nText:\n{chunk}\nKeywords:\n{keywords}."
            out = ask_gemini(request_text) 
...
def generate_kg(keywords_expander, dataset: str, keywords: List[str], chunks: List[str], enable=['none']) -> str:
    ...
    # Formatting the user's request into a single string to feed into the model
    request_text = f"You are a scientific knowledge graph constructor who is trained to generate knowledge graphs based on a strict set of criteria. Please generate knowledge graph triples based on the provided keywords and document segments. Keywords: {keywords}. Document segments: {chunks}. Criteria: {generate_kg_criteria}\n{more}"
    out = ask_gemini(request_text) 
