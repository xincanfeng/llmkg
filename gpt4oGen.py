...
def ask_openai(system_prompt: str, user_prompt: str, max_retries: int = 50, delay: float = 5.0) -> str:
    """
    Helper function to interact with the GPT-4o model via OpenAI's API.
    Retries the request if the server does not respond properly.
    """
    retries = 0
    while retries < max_retries:
        try:
            ...
            if response:
                ...
                logger.warning(f"Empty response from OpenAI API. Retrying ({retries + 1}/{max_retries})...")
                retries += 1
                time.sleep(delay)
        except Exception as e:
            logger.error(f"Error during OpenAI API call: {e}. Retrying ({retries + 1}/{max_retries})...")
            retries += 1
            time.sleep(delay)
    logger.error("Maximum retries reached. The server did not respond properly.")
    return None

# raw_answer: ChatCompletionMessage(content='According to the criteria provided, the most similar entity from the candidate list to the query entity "N" is \'nucleic acid nucleoside or nucleotide\' as it matches the query entity type.', 
#                                   refusal=None, role='assistant', function_call=None, tool_calls=None)

def eval_accuracy(not_matched_triples: List[Tuple[str, str, str]], chunks: List[str], enable=['none']) -> Tuple[List[Tuple[str, str, str]], List[Tuple[str, str, str]]]:
    """
    ...
    for gen_triple in not_matched_triples:
        ...
        system_prompt = f"You are an expert at evaluating the accuracy of scientific knowledge graph based on the provided documents segment.{add}"
        request_text = f"Based on the document segments provided, please determine the correctness of the knowledge graph triple. The knowledge graph triple:\n{gen_triple}\nThe document segment:\n{chunks}\nRespond with only one word: 'TRUE' or 'FALSE' and do not provide any explanation or reasoning."
        raw_answer = ask_openai(system_prompt, request_text)
...
def eval_confliction(similar_gt_triples_dict, chunks: List[str], enable=['none']) -> Tuple[dict, dict]:
    """
    ...
    for gt_triple, gen_triples in similar_gt_triples_dict.items():
        ...
        chunks_str = "\n".join(chunks)
        system_prompt = conflict_resolution_prompt.format(triples_str=triples_str, chunks_str=chunks_str)
        request_text = "Begin by applying the above steps carefully and provide only the final required output."
        raw_conflict_output = ask_openai(system_prompt, request_text)
        ...
        if conflict_triples:
            ...
            print(f"Parsed conflict_triples: {conflict_triples}")
        if removal_triples:
            removal_conflict_triple_dict[gt_triple] = removal_triples
            print(f"Parsed removal_triples: {removal_triples}")
...
def eval_exact_equiv(equivalent_gt_triples_dict) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
    """
    ...
    for gen_triple, equiv_gt_triples in equivalent_gt_triples_dict.items():
        ...
        for equiv_gt_triple in equiv_gt_triples:
            ...
            request_text = f"Please judge if the following two triples are exactly equivalent. Triple 1: {gen_triple}, Triple 2: {equiv_gt_triple}. Please answer by providing 'TRUE' or 'FALSE' value."
            raw_answer = ask_openai(system_prompt, request_text)
...
def eval_equivalence(similar_gt_triples_dict_w_promptlink) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
    """
    ...
    for gen_triple, similar_gt_triples in similar_gt_triples_dict_w_promptlink.items():
        ...
        for similar_gt_triple in similar_gt_triples:
            system_prompt = "You are an expert at evaluating scientific knowledge graphs equivalence who always responds by providing 'TRUE' or 'FALSE' value."
            request_text = f"Please judge if the following two triples are equivalent. Triple 1: {gen_triple}, Triple 2: {similar_gt_triple}."
            raw_answer = ask_openai(system_prompt, request_text)
...
def get_similar_entities_dict_w_promptlink(keywords_expander: str, k_similar_entities_dict_w_sapbert: Dict[str, List[str]], dataset: str) -> List[str]:
    """
    ...
    for gen_entity, similar_gt_entities in k_similar_entities_dict_w_sapbert.items():
        ...
        for similar_gt_entity in similar_gt_entities:
            system_prompt = f"You are an expert at evaluating entity similarity in scientific knowledge graphs based on these criteria:\n{filter_entity_criteria}. You always respond by providing 'TRUE' or 'FALSE' value."
            request_text = f"Please judge if the following two entities are similar. The query entity: {gen_entity}, the candidate entity: {similar_gt_entity}."
            raw_answer = ask_openai(system_prompt, request_text)
...
def get_similar_triples_dict_w_promptlink(not_matched_triples, k_similar_entities_dict_w_sapbert, dataset: str, strictness: str = 'strict', keywords_expander: str = 'none') -> Dict[str, List[str]]:
    """
    ...
    """
    similar_entities_dict = get_similar_entities_dict_w_promptlink(keywords_expander, k_similar_entities_dict_w_sapbert, dataset)
...
def eval_similar_w_promptlink(keywords_expander, similar_gt_triples_dict, dataset: str) -> Dict[Tuple[str, str, str], List[Tuple[str, str, str]]]:
    """
    ...
    for gen_triple, similar_gt_triples in similar_gt_triples_dict.items():
        ...
        for similar_gt_triple in similar_gt_triples:
            system_prompt = f"You are an expert at evaluating triples similarity in scientific knowledge graphs based on these criteria:\n{filter_entity_criteria}. You always respond by providing 'TRUE' or 'FALSE' value."
            request_text = f"Please judge if the following two triples are similar. The query triple: {gen_triple}, the candidate triple: {similar_gt_triple}"
            raw_answer = ask_openai(system_prompt, request_text)
...
def iterative_denoising_and_keyword_extraction(keywords: List[str], chunks: List[str], iteration_times: int = 3) -> Tuple[List[str], List[List[str]]]:
    """
    ...
    for iteration in range(1, iteration_times + 1):
        ...
        for chunk in tqdm(current_chunks, desc=f"Keywords Extraction Iter {iteration}"):
            system_prompt = f"You are a scientific knowledge graph constructor who is trained to transform the text into a list of important keywords based on these criteria:\n{keyword_extraction}."
            request_text = f"Please transform the text into a list of important keywords.\nText:\n{chunk}."
            out = ask_openai(system_prompt, request_text)
        ...
        for chunk, keywords in tqdm(zip(current_chunks, current_keywords), desc=f"Denoising Iter {iteration}", total=len(current_chunks)):
            denoised_chunks = []
            system_prompt = f"You are a scientific knowledge graph constructor who is trained to denoise the raw text with the given keywords based on these criteria:\n{chunks_denoising}."
            request_text = f"Please denoise the raw text with the given keywords.\nText:\n{chunk}\nKeywords:\n{keywords}."
            out = ask_openai(system_prompt, request_text)
...
def generate_kg(keywords_expander, dataset: str, keywords: List[str], chunks: List[str], enable=['none']) -> str:
    """
    ...
    if 'denoising_doc' in enable:
        keywords, chunks = iterative_denoising_and_keyword_extraction(keywords, chunks)

    system_prompt = "You are a scientific knowledge graph constructor who is trained to generate knowledge graphs based on a strict set of criteria."
    request_text = f"Please generate knowledge graph triples based on the provided keywords and document segments. Keywords: {keywords}. Document segments: {chunks}. Criteria: {generate_kg_criteria}\n{more}"
    out = ask_openai(system_prompt, request_text)
